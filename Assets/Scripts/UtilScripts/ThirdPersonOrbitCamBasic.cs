...
public class ThirdPersonOrbitCamBasic : MonoBehaviour 
{
	...
	void Update()
	{
		...
		RaycastHit hit;
		if (Physics.Raycast(rayStart.position, rayTarget.forward, out hit,1000f, ~ignoreMask))
		{
			if (hit.collider)
			{
	...
	bool ViewingPosCheck (Vector3 checkPos, float deltaPlayerHeight)
	{
		...
		if (Physics.SphereCast(checkPos, 0.2f, target - checkPos, out RaycastHit hit, relCameraPosMag, ~ignoreMask))
		{
	...
	bool ReverseViewingPosCheck(Vector3 checkPos, float deltaPlayerHeight, float maxDistance)
	{
		...
		if (Physics.SphereCast(origin, 0.2f, checkPos - origin, out RaycastHit hit, maxDistance, ~ignoreMask))
		{
